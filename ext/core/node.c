...
#define INDEX_LIST_ENTRY_CAPA 8
typedef struct {
  uint32_t len;
  uint32_t next;
...
index_list_request_entry(IndexList *index_list, uint32_t *index) {
...
index_list_append(IndexList *index_list, st_data_t pair) {
  ...
  uint32_t index = PAIR64_SECOND(pair);

  // new entry
  if(index == UINT32_MAX) {
    uint32_t new_index;
    IndexListEntry *new_entry = index_list_request_entry(index_list, &new_index);
    new_entry->next = UINT32_MAX;
    new_entry->len = 1;
    new_entry->data[0] = value;
    return new_index;
  } else {
    IndexListEntry *entry = &index_list->entries[index];
...
static inline void
node_diff_step(uint32_t iold, uint32_t inew, st_table *_overlap, st_table *overlap,
          uint32_t start_old, uint32_t start_new,
          uint32_t *sub_length, uint32_t *sub_start_old, uint32_t *sub_start_new) {
  ...
  if(len_old > 0) {
    for(size_t inew = start_new; inew < start_new + len_new; inew++) {
      ...
      if(st_lookup(old_index_map, (st_data_t)&key, &pair)) {
        ...
        node_diff_step(value, inew, _overlap, overlap, start_old, start_new,
                  &sub_length, &sub_start_old, &sub_start_new);

        if(next_index != UINT32_MAX) {
          IndexListEntry *entry = &index_list->entries[next_index];
          IndexListEntry *iter = entry;
          while(true) {
            for(uint32_t i = 0; i < iter->len; i++) {
              node_diff_step(iter->data[i], inew, _overlap, overlap, start_old, start_new,
                        &sub_length, &sub_start_old, &sub_start_new);
            }
