...
static VALUE
rb_php_initialize(int argc, VALUE *argv, VALUE self) {
  ID language_id = rb_intern("@__language__");
  VALUE rb_language_func = ULL2NUM((uintptr_t) tree_sitter_php);
...
void Init_php()
{
  ...
  rb_define_method(rb_cPhp, "initialize", rb_php_initialize, -1);
}
